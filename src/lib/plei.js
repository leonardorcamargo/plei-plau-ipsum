import { pleiQuotes } from './quotes';

export class PleiIpsum {
  constructor(options) {
  ...
  createOneParagraph() {
    ...
    for (let i = 0; i < options.pQuotes; i++) {
      const thisPosition =
        Math.round(Math.random() * (randomLimit - 1) + 1) - 1; // get a random position
...
export const pIpsum = options => new PleiIpsum(options).init();
